...
tab_theory, tab_practice, tab_archive = st.tabs(["📖 Правила", "✏️ Задачи", "🗂️ Архив PDF"])
...
with tab_archive:
    st.subheader("Архив материалов")
    st.caption("Файлы разбиты по темам и подтемам внутри `archive_pdfs`.")

    theme_pdfs = _collect_theme_pdfs(ARCHIVE_DIR)
    if not theme_pdfs:
        st.info(
            "Архив пока пуст. Создайте структуру вроде "
            "`archive_pdfs/5класс/геометрия/*.pdf` и `archive_pdfs/5класс/дроби/*.pdf`."
        )
    else:
